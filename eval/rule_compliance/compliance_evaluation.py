...
def load_output_csv(model, question_type, overwrite_answers=False):
    ...
    if not os.path.exists(csv_name) or overwrite_answers:
        questions_pd = pd.read_csv(f"../../dataset/rule_compliance/rule_{question_type}_qa.csv")
...
def run_thread(model, question, image_path, context):
    ...
    question = add_context_to_prompt(question, context) # TODO: verify that this is working as expected
...
def save_results(model, macro_avg, definitions_avg, multi_avg, single_avg, all_answers, question_type):
    print(f"\nMacro avg: {macro_avg}")
...
if __name__ == '__main__':
    ...
    for question_type in ['dimension', "functional_performance"]:
        for model in ['gpt-4-vision-preview', 'llava-13b']:
            ...
            for i, row in tqdm(questions_pd.iterrows(), total=len(questions_pd), desc=f'generating responses for '
                ...
                image_path = f"../../dataset/rule_compliance/rule_{question_type}_qa/" + row['image']
            ...
            macro_avg, definitions_avg, multi_avg, single_avg, all_answers = eval_qa(csv_name)  # TODO: change to approapriate evaluation based on question_type

            # Print and save the results
            save_results(model, macro_avg, definitions_avg, multi_avg, single_avg, all_answers, question_type)
