...
def load_output_csv(model, question_type, overwrite_answers=False):
    ...
    if not os.path.exists(csv_name) or overwrite_answers:
        questions_pd = pd.read_csv(f"../../dataset/rule_compliance/rule_dimension_qa/{question_type}/dimension_idealrag1.csv")
...
def run_thread(model, question, image_path, context):
    if model == 'llava-13b' or model == 'llava-13b+idealRAG':
    ...
    print("up to run complete response")
