...
def run_thread(model, question, image_path, context):
    if model == 'llava-13b':
        ...
        multi_modal_llm = AnthropicMultiModal(model="claude-3-opus-20240229", max_tokens=300)
    ...
    for question_type in ["context", "detailed_context"]:
        for model in model_list:
            ...
            for i, row in tqdm(questions_pd.iterrows(), total=len(questions_pd), desc=f'generating responses for '
                ...
                # Run through model
                if model in ['gpt-4-1106-vision-preview+RAG', 'llava-13b', 'fuyu-8b', 'gemini-pro', 'claude-opus-RAG']:
