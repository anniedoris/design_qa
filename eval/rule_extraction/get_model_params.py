...
def load_output_csv(model, question_type, overwrite_answers=False):
    ...
    csv_name = f"{question_type}_evaluation_{model}_model_params.csv"
...
def run_thread(model, question, context):
    ...
    if model in ['llava-13b', 'gpt-4-1106-vision-preview', 'gpt-4-1106-vision-preview+RAG', 'llava-v1.6', 'llava-13b+idealRAG']:
...
if __name__ == '__main__':
    ...
    for question_type in ['retrieval']:
        ...
        for model in ['llava-13b+idealRAG']:
            ...
            questions_pd = pd.read_csv('../../dataset/rule_extraction/retrieval_idealrag1.csv')
            ...
            for i, row in tqdm(questions_pd.iterrows(), total=len(questions_pd), desc=f'generating responses for '
                ...
                elif model in ['llava-13b+idealRAG']:
                    context = ''
                else:
                    raise ValueError("Invalid model")
                try:
