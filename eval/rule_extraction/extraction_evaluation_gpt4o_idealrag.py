...
def run_thread(model, question, context):
    ...
    elif model == 'llava-13b' or model == 'llava-13b+idealRAG':
        # os.environ["REPLICATE_API_TOKEN"] = ""
    ...
    elif model in ['gpt-4-1106-vision-preview', 'gpt-4-1106-vision-preview+RAG', 'gpt-4o+idealRAG']:
        # OpenAI model
        llm = OpenAIMultiModal(model="gpt-4o", max_new_tokens=250)
    ...
    if model in ['llava-13b', 'gpt-4-1106-vision-preview', 'gpt-4-1106-vision-preview+RAG', 'llava-v1.6', 'llava-13b+idealRAG', 'gpt-4o+idealRAG']:
...
if __name__ == '__main__':
    ...
    for question_type in ['retrieval']:
        ...
        for model in ['gpt-4o+idealRAG']:
            # questions_pd, csv_name = load_output_csv(model, question_type, overwrite_answers)
            questions_pd = pd.read_csv('../../dataset/rule_extraction/ideal_rag_set1_131onward.csv')
            csv_name = f"{question_type}_evaluation_{model}.csv"
            ...
            for i, row in tqdm(questions_pd.iterrows(), total=len(questions_pd), desc=f'generating responses for '
                ...
                # TODO: this may have conflict with top_k = 0, running allrules using this code
                elif model in ['llava-13b+idealRAG', 'gpt-4o+idealRAG']:
                    context = ''
                else:
                    raise ValueError("Invalid model")
                try:
